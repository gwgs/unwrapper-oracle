...
public class Unwrapper {
    ...
    public static String unwrap(final String wrapped) throws DataFormatException, IOException, NoSuchAlgorithmException {
        ...
        final Pattern lengthPattern = Pattern.compile("([\n][0-9a-f]+[ ])([0-9a-f]+[\n])");
        final Matcher m = lengthPattern.matcher(wrappedUnix);
        if (!m.find(0)) {
        ...
        final int encodedCodeLength = Integer.parseInt(m.group(2).trim(), 16);
        final int expectedLength = m.end() + encodedCodeLength;
        ...
        final String encoded = wrappedUnix.substring(m.end(), expectedLength);
        final byte[] decoded = Base64Coder.decodeLines(encoded);
        final byte[] remapped = new byte[decoded.length];

        for (int i = 0; i < decoded.length; ++i) {
            final int unsignedInteger = decoded[i] & 255;
